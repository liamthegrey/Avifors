...
// block style.css of all themes!
//load increase with hide wp-login.php
//social login compatibility done
   ...
   */
   function __construct() {
    ...
    function access_cookie(){
        return preg_replace("/[^a-zA-Z]/", "", substr(NONCE_SALT, 0, 6));
    }
    /**
        ...
        if ($replace_lines) {
            foreach ($replace_lines as $line) {
                ...
                if (isset($replace_word[0]) && isset($replace_word[1])) {
                    ...
                    if ($type=='replace_in_html'){
                        $output .='<label><input type="radio" '.$replace_checked.' class="html_actiontype" value="replace" name="html_actiontype_'.$rand.'" class="radio">Replace</label>
<br>';
                        $output .= '<label><input type="radio" '.$remove_checked.' class="html_actiontype" value="remove" name="html_actiontype_'.$rand.'" class="radio">Remove</label>
</div>';
                    }else{
                        $output .='<label><input type="radio" '.$replace_checked.' class="url_actiontype" value="replace" name="urls_actiontype_'.$rand.'" class="radio">Replace</label>
<br>';
                        $output .= '<label><input type="radio" '.$remove_checked.' class="url_actiontype" value="remove" name="urls_actiontype_'.$rand.'" class="radio">Hide (404)</label>
</div>';
                    }

                        $output .=  '<textarea style="visibility:'.$remove_hidden.'" name="'.$type.'2[]" class="second_field"/>'.$replace_word[1].'</textarea>';

                    $output .= '<a href="javascript:void(0);" class="button hmwp_action hmwp_remove_button" title="Remove Rule"><img src="'.$this::url.'/img/delete.png" width="12"/>
                  </a>
';
...
.hmwp_action{margin: 4px !important;}
        ...
        $output .= '<a href="javascript:void(0);" class="button hmwp_action htmwp_add_button " title="Add Rule">
                               <img src="'.$this::url.'/img/add.png" width="12" />
                               Add
                          </a>';
        $output .=  '</div>';
...
if (!is_admin()) {
	...
	echo "\n".'testcontent:'. get_option('test');
        ...
        if ($this->opt('admin_key'))
            $this->trust_key = '?p_'.$login_query.'='.$this->opt('admin_key');
        else
            $this->trust_key = '';
...
';
        ...
        if ($this->opt('avoid_direct_access')){
            ...
            $output.='#Remove # from following two lines and add them to the top of location ~ \.php$ block:';
    ...
    function iis_config(){
        ...
        if ($new_content_path)
            $output.='<rule name="HMWP Content'.rand(0,200).'" stopProcessing="true">'."\n\t".'<match url="^'.$new_content_path. '/(.*)"  />'."\n\t".'<action type="Rewrite" url="'. $rel_content_path. '/{R:1}"  appendQueryString="true" />'."\n".'</rule>'."\n";
